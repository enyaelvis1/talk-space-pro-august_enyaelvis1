-- Make the booking hold RPC explicit about the row state it writes.
--
-- Some live databases have drifted far enough that relying on the table
-- default for `status` is no longer safe during the booking insert. Writing
-- the hold state explicitly keeps the insert compatible with the current
-- appointment checks and protects the booking flow from schema drift.
