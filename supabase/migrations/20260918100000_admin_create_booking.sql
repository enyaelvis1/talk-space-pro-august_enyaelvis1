-- Allow administrators to create an appointment without bypassing the same
-- service, availability, payment, package, and confirmed-slot safeguards.
...
begin
  ...
  select * into therapist from public.therapists
  where id = p_therapist_id and is_active;
  if not found then
  ...
  if p_package_id is not null then
    ...
    select * into pkg from public.client_session_packages
  ...
  if p_package_id is null and p_payment_method <> 'unpaid' then
    ...
    insert into public.payments (
      appointment_id, provider, reference, amount_kobo, status, metadata, verified_by, verified_at, created_by
    ) values (
  ...
  if appointment_status = 'confirmed' then
    ...
    set status = 'confirmed', hold_expires_at = null,
        paid_amount_kobo = case when p_package_id is not null then 0 else greatest(coalesce(p_amount_kobo, 0), 0) end,
        payment_reference = case when p_package_id is not null then 'PACKAGE-' || upper(substr(p_package_id::text, 1, 8)) else payment_reference_value end,
        updated_at = now()
    where id = appointment_id;
  end if;

  return query
  select a.id, a.booking_reference, a.status, a.payment_reference, a.package_id, a.starts_at, a.ends_at
  from public.appointments a where a.id = appointment_id;
end;
$$;

revoke all on function public.create_admin_appointment(uuid, text, text, text, uuid, uuid, public.session_mode, timestamptz, text, text, text, text, bigint, uuid) from public, anon;
grant execute on function public.create_admin_appointment(uuid, text, text, text, uuid, uuid, public.session_mode, timestamptz, text, text, text, text, bigint, uuid) to authenticated, service_role;
