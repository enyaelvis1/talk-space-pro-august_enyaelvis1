-- Preserve grouped appointment checkout confirmation after the package-payment
-- migration redefined mark_payment_status for purchase-first payments.
...
BEGIN
  ...
  IF p.status = 'refunded' OR (p.status = 'succeeded' AND p_new_status <> 'refunded') THEN
    RETURN p;
  END IF;
