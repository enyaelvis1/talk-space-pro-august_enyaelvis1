-- A retried reschedule request must not move the same appointment twice or
-- trigger another downstream Google/email reconciliation.
...
BEGIN
  ...
  -- The same successful request can be safely replayed. Return the committed
  -- row without changing its timestamp, timeline, slot, or notification state.
  IF appt.status = 'confirmed'
     AND appt.starts_at = p_new_starts_at
     AND appt.therapist_id = target_therapist
     AND appt.session_mode = target_mode THEN
    id := appt.id;
